...
class VisionaryFairMP:
    @staticmethod
    def compute(
        pool: list[structs.StudentMeta],
        ...
        for i in range(pool_size):
            ...
            if student.multiplier <= 0:
                raise ValueError(
                    f'学生 {student.id} 的倍率 {student.multiplier} 必须为正的有限值'
                )

            count = draw_count_by_id.get(student.id, 0)
            if count <= 0:
                raise ValueError(f'学生 {student.id} 的次数为负')

            draw_count[i] = count
            multiplier_sum += student.multiplier
        ...
        for slot in range(dimension_count):
            ...
            for i in range(pool_size):
                ...
                if dimension.dimension_id >= len(labels):
                    raise ValueError(
                        f'学生 {pool[i].id} 缺少维度 {dimension.dimension_id} 的标签'
                    )

                label = labels[dimension.dimension_id]
                if label < 0:
                    raise ValueError(f'学生 {pool[i].id} 的标签为负')
