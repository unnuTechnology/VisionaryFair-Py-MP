...
class ItemMeta:
    ...
    Arguments:
        ...
        max_pick: 本周期项目最多被抽到的次数，作为安全阀。为 ``0`` 代表不限次数。
...
class DimensionBalance(NamedTuple):
    ...
    Arguments:
        dimension_id: 维度编号，对应 ``ItemMeta.labels`` 的下标。
...
class WeightSettings:
    ...
    Arguments:
        ...
        lowest_rnd: 保底份额。任何池内项目的最低被抽概率 = 该值 / 池内人数。它也是交替率的精确调节器: P(A) = 1/2 + (1-e)² / (2(2-e)), 误差 ±0.002。注意池很小时保底会变大 (剩 10 人时每人 1%), 此时权重的影响被稀释。
